...
class NeuroDyn_Neuron:
    ...
    def __init__(self):
        ...
        # When scaled by I_master, this gives Ib0 to Ib6
    ...
    def perturb(self,sigma=0.15):
        r = np.random.randn()
        self.I_master += self.I_master*sigma*r
        r = np.random.randn()
        self.I_ref +=  self.I_ref*sigma*r
        # do we need different I_refs?
        # what about I_voltage?
        self.pNa += 0.2*np.random.randn()
        self.qNa += 0.1*np.random.randn()
        self.pK += 0.2*np.random.randn()
    ...
    # Sodium current
    def I_Na(self, V, m, h):
        return self.g[0] * m**self.pNa * h**self.qNa * (V - self.E_rev[0])
    # Postassium current
    def I_K(self, V, n):
        return self.g[1] * n**self.pK * (V - self.E_rev[1])
    # Passive current
    def I_L(self, V):
    ...
    def simulate_single_neuron(self, init_conds, time_len):
        # This will call all the functions necessary to simulate the single neuron
        A = self.solve_odes(self.dXdtA, init_conds, time_len, plot=True)
        return A
...
#%%
neuron = NeuroDyn_Neuron()
neuron.simulate_single_neuron([0,0,0,0],0.01)